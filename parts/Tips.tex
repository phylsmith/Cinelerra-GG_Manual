...
You can optimize RAM utilization with \textit{Cache size} and \textit{Seconds to preroll render} parameters. See section \ref{sec:cache_and_preroll}. If there is limited RAM you must necessarily have a large swap partition. For system swap, (2 x RAM) GB seems to be more than sufficient. If the amount of memory being used by the program is close, then swap might save you but often if swapping becomes necessary, it presents more problems and you end up killing the \CGG{} process anyway.
	...
	\item Switching between multiple GPUs using \textit{switcherooctl} is useful for determining which GPU is most beneficial for processing.
\end{itemize}
...
under \hyperref[sub:gpu_hardware_encoding]{GPU hardware encoding}. Decode operations are described here next.  Decode options are located in:

\texttt{Settings $\rightarrow$ Preferences... $\rightarrow$ Tab Performance $\rightarrow$ Use HW Device}. 
...
Vulkan requires initialization of its device type. This is done automatically by \CGG{} for decoding but still needs to be made explicit in the encoding presets. (\texttt{CIN\_HW\_DEV=vulkan}). Drivers and environment variables are described below:
\begin{itemize}
	\item For \textbf{Intel}: Install \textit{ANV} Open Source driver. For configuration: \texttt{ANV\_DEBUG=\\video-decode,video-encode}.
	\item For \textbf{Nvidia}: Only \textit{GTX 10xx+} (Pascal) graphic cards and driver (closed) $550.40.07+$. For Noveau driver (open), install \textit{noveau-vulkan}.
	\item For \textbf{AMD}: RDNA+ (VCN2+) graphic cards; driver RADV, Mesa 285+. For pre-VCN2 graphic cards: \texttt{RADV\_PERFTEST=video\_decode,video\_encode}.
\end{itemize}

AppImage will probably not allow for either VDPAU, VA-API or Vulkan hardware acceleration because the
...
\section{Switching between multiple GPUs}%\protect\footnote{credit to Terje}
\label{sec:switcherooctl}

The current standard tool for switching between multiple gpus on most Linux distributions
is \textit{switcherooctl}. It acts as a userspace daemon to switch GPUs on the fly. This
makes it so you can run specific applications (like \CGG{}, games, or browsers) on the dedicated
GPU using:

\hspace*{1cm}\texttt{switcherooctl launch <command>}
...
Once the \texttt{switcheroo-control} service is running, you can find options to launch applications on dedicated or integrated graphics by right-clicking application icons and select \textit{"Launch Using Dedicated Graphics Card"}.
Its big advantage is that there is no need to log out in order to switch gpus. It supports both NVIDIA (proprietary, Optimus) and AMD/Intel hybrid, combo graphics. 

Typical use case: Launch \CGG{} or FFmpeg on the most powerful dedicated dGPU card, and let the rest of the desktop use the integrated iGPU.

\textbf{1.} Install and  enable the service switcheroo-control service. For example on openSUSE Tumbleweed-Slowroll:

\hspace*{1cm}\texttt{sudo zypper install switcheroo-control} \newline
\hspace*{1cm}\texttt{sudo systemctl enable --now switcheroo-control.service}

\textbf{2.} Verify available GPUs (for example on a legacy Intel Skylake workstation with iGPU and Nvidia dGPU card):
...
\textbf{3.} Verify if Nvidia's proprietary driver is installed and working by launching \textit{nvidia-smi}:

\begin{figure}[htpb]
	\centering
	\includegraphics[width=0.8\linewidth]{nvidia-smi.png}
		\label{fig:nvidia-smi}
\end{figure}

\textbf{4.} Verify if Vulkan drivers are installed and working using vkcube demo (part of vulkan-tools):

\textit{4.1} On Intel iGPU (default):
...
\textbf{5.} Launch \CGG{} using Nvidia dGPU:

\begin{lstlisting}[style=sh]
	switcherooctl launch -g 1 cin
	
	Cinelerra Infinity - built: Jan 20 2026 06:03:54
	
	file to render to
	/home/xxx/Videos/hdv09_04_cin_h264_nvenc.mp4
	...
	** rendered 5972 frames in 57.158 secs, 104.482 fps
	audio0 pad 64 0 (64)
\end{lstlisting}
...
\begin{itemize}
	...
	\item When working with multiple tracks containing video at opacity 100\%, if you just want to play one of the tracks, disable "Play track" in the patchbay for the tracks that you do not need to see.  This uses a lot less CPU.
	\item For playing a track that has attached Video Effects, if you just want to play the track, you can temporarily disable the Video Effects which will decrease CPU considerably in most cases.
\end{itemize}
